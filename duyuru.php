...
$sorgu=mysql_query("SELECT * FROM duyurular WHERE id='$_GET[page]'");
$bilgi=mysql_fetch_array($sorgu);
{
	mysql_query("UPDATE duyurular SET hit=hit+1 WHERE id='$_GET[page]'");
	echo"<ul>";
	
	echo"<i class=\"fa fa-user-circle fa-lg\"><a href=\"#\">{$bilgi[bolum_bask]}</a></i>";
	echo"<br/>";
	echo"<br/>";
...
<div class="col-md-3 enson ">
<h3>
<i class="fa fa-bullhorn" aria-hidden="true"><a href="#">Son Bölüm Duyuruları</a></i>
</h3>
<hr/>
<?php
$sorgu=mysql_query("select * from duyurular order by  id desc, isim LIMIT 0,4");
	while($bilgi=mysql_fetch_array($sorgu))
     {
		 echo"<i class=\"fa fa-tags\"title=\"konu adı\"><a href=\"duyuru.php?page={$bilgi[id]}\">{$bilgi[isim]}</a></i>";
		 echo "<br>";
